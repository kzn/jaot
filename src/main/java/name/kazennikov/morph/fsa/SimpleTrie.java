...
import name.kazennikov.morph.fsa.Trie.Node;
 ...
 */
public class SimpleTrie<In, FC extends Collection<Final>, Final> {
	Trie.Node<In, FC, Final> start;
	List<Trie.Node<In, FC, Final>> nodes = new ArrayList<Trie.Node<In, FC, Final>>();
	
	Stack<Trie.Node<In, FC, Final>> free = new Stack<Trie.Node<In, FC, Final>>();
	
	Register reg = new Register();
	
	class Register {
		HashMap<Node<In, FC, Final>, Node<In, FC, Final>> m = new HashMap<Trie.Node<In,FC, Final>, Trie.Node<In,FC, Final>>();
	...
	public SimpleTrie(Trie.Node<In, FC, Final> start) {
		this.start = start;
		nodes.add(start);
	...
	public void add(List<In> seq, Final fin) {
		Trie.Node<In, FC, Final> current = start;
		
		int idx = 0;
		
		while(idx < seq.size()) {
			Trie.Node<In, FC, Final> n = current.getNext(seq.get(idx));
			if(n == null)
				break;
	 ...
	 * @return next node
	 */
	public Trie.Node<In, FC, Final> getNextOrAdd(Trie.Node<In, FC, Final> node, In in) {
		Trie.Node<In, FC, Final> next = node.getNext(in);
	...
	public FC get(List<In> seq) {
		...
		for(In in : seq) {
			Trie.Node<In, FC, Final> next = n.getNext(in);
	 ...
	 * @param fin final state
	 */
	protected List<Node<In, FC, Final>> addSuffix(Trie.Node<In, FC, Final> n, List<In> seq, Final fin) {
		Trie.Node<In, FC, Final> current = n;
		
		List<Node<In, FC, Final>> nodes = new ArrayList<Trie.Node<In, FC, Final>>();
		
		for(In in : seq) {
			Trie.Node<In, FC, Final> node = makeNode();
			nodes.add(node);
	...
	public void addFinal(Trie.Node<In, FC, Final> node, Final fin) {
	...
	public void setNext(Trie.Node<In, FC, Final> src, In in, Node<In, FC, Final> dest) {
		reg.remove(src);
		src.setNext(in, dest);
	}
	 ...
	 * @return
	 */
	public Trie.Node<In, FC, Final> getStart() {
		return start;
	}
	 ...
	 * @return
	 */
	protected Trie.Node<In, FC, Final> makeNode() {
		if(!free.empty())
			return free.pop();
		
		Trie.Node<In, FC, Final> node = start.makeNode();//new Trie.SimpleNode<In, Final>();
		nodes.add(node);
		node.setNumber(nodes.size());
		return node;
	}
	
	protected Trie.Node<In, FC, Final> cloneNode(Trie.Node<In, FC, Final> src) {
		Trie.Node<In, FC, Final> node = makeNode();
		src.assign(node);
		return node;
	}
	...
	public boolean isConfluence(Trie.Node<In, FC, Final> node) {
		return node.inbound() > 1;
	}
	
	List<Node<In, FC, Final>> commonPrefix(List<In> input) {
		Node<In, FC, Final> current = start;
		List<Node<In, FC, Final>> prefix = new ArrayList<Trie.Node<In, FC, Final>>();
		prefix.add(current);
	...
	public void addMinWord(List<In> input, Final fin) {
		...
		List<Node<In, FC, Final>> nodeList = new ArrayList<Trie.Node<In,FC, Final>>(prefix);
	...
	public void write(Trie.Writer<In, Final> writer) throws IOException {
...
//	public static void main(String[] args) throws IOException, SAXException, ParserConfigurationException, RecognitionException {
...
//		SimpleTrie<Character, Integer> fsa = new SimpleTrie<Character, Integer>();
//		SimpleTrie<Character, Integer> trie = new SimpleTrie<Character, Integer>();
//		
//		Terms.terms().load("term.xml");		
//		MorphConfig mc = MorphConfig.build("russian.des");
//
//		
//		ParadigmReader reader = new ParadigmReader("wf.txt.gz", mc.terms());
//		
//		TObjectIntHashMap<List<String>> endings = new TObjectIntHashMap<List<String>>();
//		
//		
//		int n = 0;
//		long start = System.currentTimeMillis();
//		for(List<WordForm> par : reader) {
//			boolean skip = false;
//			for(WordForm wf : par) {
//				if(wf == null)
//					throw new IllegalStateException();
//
//				if(wf.getLemma().equals("РОВЕНСКИЙ2")) {
//					skip = true;
//					break;
//				}
//				
//				if(wf.getMorphemes().isEmpty())
//					throw new IllegalStateException();
//				
//				for(WordForm.Morpheme m: wf.getMorphemes())
//					if(m == null)
//						throw new IllegalStateException();
//				
//			}
...
//			
//			for(WordForm wf : par) {
//				int end = getId(endings, wf.getFeats());
//				List<Character> w = string2CharList(wf.getWordform());
//				fsa.addMinWord(w, end);
//				//trie.add(w, end);
//				
//				//fsa.toDot("fsa.dot");
//				//trie.toDot("trie.dot");
...
//			
//			/*if(n % 1000 == 0) {
//				if(!fsa.getStart().equiv(trie.getStart())) {
//					System.out.printf("FSA not equiv to trie when adding word%n");
//					throw new IllegalStateException();
//				}
//
//			
...
//			
//			if(++n % 1000 == 0) {
//				System.out.printf("%d words... trie:%d, fsa:%d, elapsed %d ms%n", 
//						n, trie.size(), fsa.size(), System.currentTimeMillis() - start);
//			}
//			
//			
//		}
//		if(trie.getStart().equiv(fsa.getStart())) {
//			System.out.printf("FSAs are equal%n");
//		}
//		
//		
